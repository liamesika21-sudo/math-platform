...
import { exam2022BQuestions, examInfo2022B } from '@/data/exam-2022b';

export default function Exam2022BPage() {
  ...
  const filteredQuestions = useMemo(() => {
    return exam2022BQuestions.filter(q => {
      ...
      if (searchQuery) {
        ...
        return (
          ...
          q.questionTypeHe.includes(searchQuery)
        );
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-500 mt-1">פתרונות מלאים עם הסברים מפורטים, סוג השאלה ותהליך החשיבה</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-gray-100 p-4 text-center">
          ...
          <p className="text-2xl font-bold text-gray-900">{examInfo2022B.shortQuestions + examInfo2022B.openQuestions}</p>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 text-center">
          ...
          <p className="text-2xl font-bold text-gray-900">{examInfo2022B.shortQuestions}</p>
          <p className="text-sm text-gray-500">שאלות קצרות ({examInfo2022B.shortQuestionPoints} נק')</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4 text-center">
          <div className="w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center mx-auto mb-2">
            <BookOpen className="w-5 h-5 text-purple-600" />
          </div>
          <p className="text-2xl font-bold text-gray-900">{examInfo2022B.openQuestions}</p>
          <p className="text-sm text-gray-500">שאלות פתוחות ({examInfo2022B.openQuestionPoints} נק')</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4 text-center">
          <div className="w-10 h-10 bg-orange-100 rounded-lg flex items-center justify-center mx-auto mb-2">
            <Clock className="w-5 h-5 text-orange-600" />
          </div>
          <p className="text-2xl font-bold text-gray-900">{examInfo2022B.duration}</p>
      ...
      {(selectedType === 'all' || selectedType === 'short') && filteredQuestions.some(q => q.type === 'short') && (
        <div className="space-y-4">
          <h2 className="font-bold text-lg text-gray-900 bg-gradient-to-l from-green-50 to-emerald-50 px-4 py-3 rounded-xl border border-green-100">
            חלק א' - שאלות קצרות ({examInfo2022B.shortQuestionPoints} נקודות כל אחת)
          </h2>
      ...
      {(selectedType === 'all' || selectedType === 'open') && filteredQuestions.some(q => q.type === 'open') && (
        <div className="space-y-4">
          <h2 className="font-bold text-lg text-gray-900 bg-gradient-to-l from-purple-50 to-indigo-50 px-4 py-3 rounded-xl border border-purple-100">
            חלק ב' - שאלות פתוחות ({examInfo2022B.openQuestionPoints} נקודות כל אחת)
          </h2>
      ...
      <div className="bg-gradient-to-l from-indigo-600 to-purple-600 rounded-xl p-6 text-white">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h3 className="font-bold text-lg mb-2">טיפים למבחן 2022</h3>
            <ul className="text-indigo-100 space-y-1 text-sm">
              <li>• שימי לב לסוג השאלה - זה יעזור לך להבין מה נדרש</li>
              <li>• קראי את "על מה לחשוב" לפני שאת מתחילה לפתור</li>
              <li>• בשאלות קצרות - אין צורך בהסברים אלא אם התבקשת</li>
              <li>• בשאלות פתוחות - הקפידי על כתיבה מסודרת וברורה</li>
              <li>• השתמשי בדף הנוסחאות - הוא חלק מהמבחן!</li>
            </ul>
          </div>
        </div>
      </div>
