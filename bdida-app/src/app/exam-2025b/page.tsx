...
import { exam2025BQuestions, examInfo2025B } from '@/data/exam-2025b';
...
export default function Exam2025BPage() {
  ...
  const filteredQuestions = useMemo(() => {
    return exam2025BQuestions.filter(q => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-gray-100 p-4 text-center">
          ...
          <p className="text-2xl font-bold text-gray-900">{examInfo2025B.shortQuestions + examInfo2025B.openQuestions}</p>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 text-center">
          ...
          <p className="text-2xl font-bold text-gray-900">{examInfo2025B.shortQuestions}</p>
          <p className="text-sm text-gray-500">שאלות קצרות ({examInfo2025B.shortQuestionPoints} נק')</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4 text-center">
          <div className="w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center mx-auto mb-2">
            <BookOpen className="w-5 h-5 text-purple-600" />
          </div>
          <p className="text-2xl font-bold text-gray-900">{examInfo2025B.openQuestions}</p>
          <p className="text-sm text-gray-500">שאלות פתוחות ({examInfo2025B.openQuestionPoints} נק')</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4 text-center">
          <div className="w-10 h-10 bg-orange-100 rounded-lg flex items-center justify-center mx-auto mb-2">
            <Clock className="w-5 h-5 text-orange-600" />
          </div>
          <p className="text-2xl font-bold text-gray-900">{examInfo2025B.duration}</p>
      ...
      {(selectedType === 'all' || selectedType === 'short') && filteredQuestions.some(q => q.type === 'short') && (
        <div className="space-y-4">
          <h2 className="font-bold text-lg text-gray-900 bg-gradient-to-l from-green-50 to-emerald-50 px-4 py-3 rounded-xl border border-green-100">
            חלק א' - שאלות קצרות ({examInfo2025B.shortQuestionPoints} נקודות כל אחת)
          </h2>
      ...
      {(selectedType === 'all' || selectedType === 'open') && filteredQuestions.some(q => q.type === 'open') && (
        <div className="space-y-4">
          <h2 className="font-bold text-lg text-gray-900 bg-gradient-to-l from-purple-50 to-indigo-50 px-4 py-3 rounded-xl border border-purple-100">
            חלק ב' - שאלות פתוחות ({examInfo2025B.openQuestionPoints} נקודות כל אחת)
          </h2>
      ...
      <div className="bg-gradient-to-l from-indigo-600 to-purple-600 rounded-xl p-6 text-white">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h3 className="font-bold text-lg mb-2">טיפים למבחן 2025</h3>
