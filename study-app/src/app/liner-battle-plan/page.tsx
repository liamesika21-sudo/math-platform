...
interface Theorem { id:number; name:string; prob:number; conditions:string[]; flow:string[]; source:string; }
const theorems: Theorem[] = [
  {
    id:1, name:'Rank-Nullity', prob:95,
    conditions:['A ∈ M_{m×n}(F)'],
    flow:['דרג A לצורה קנונית C (k כניסות מובילות, n-k משתנים חופשיים)','בנה בסיס ל-Nul(A): שים 1 בכל משתנה חופשי לסירוגין','קבל n-k וקטורים בת"ל (1 במקום שונה בכל אחד)','אלה פורשים את Nul(A) → dim(Nul A) = n-k','לכן: rank(A) + dim(Nul A) = k + (n-k) = n ∎'],
    source:'הרצאות 15-16 · לא נבחן במועד א\'!',
  },
  {
    id:2, name:'טבלת ההפיכות', prob:90,
    conditions:['A ∈ Mₙ(F) (ריבועית)'],
    flow:['A הפיכה ⟺ rank(A)=n','⟺ det(A)≠0','⟺ עמודות A בת"ל','⟺ עמודות A בסיס ל-Fⁿ','⟺ Ax=0 רק פתרון טריוויאלי','⟺ Nul(A)={0}','⟺ Col(A)=Fⁿ','בנה שרשרת מעגלית — כל חץ עם נימוק קצר ∎'],
    source:'הרצאות 16-18 · הופיע בכל מועד ב\'',
  },
  {
    id:3, name:'3 המשפטים (בסיס/ת"ל/פורש)', prob:85,
    conditions:['dim(V)=n','{v₁,...,vₙ} ⊆ V (בדיוק n וקטורים)'],
    flow:['(1)→(2): מהגדרת בסיס','(2)→(3): בשלילה — אם לא פורשים, ∃v∉Span → {v₁,...,vₙ,v} בת"ל (n+1 וקטורים), סתירה ל-dim=n','(3)→(1): בשלילה — אם ת"ל, ∃vᵢ שמיותר → n-1 פורשים, סתירה ל-dim=n ∎'],
    source:'הרצאות 13-14',
  },
  {
    id:4, name:'det(AB)=det(A)·det(B)', prob:75,
    conditions:['A,B ∈ Mₙ(F)'],
    flow:['מקרה 1: B לא הפיכה → rank(B)<n → rank(AB)≤rank(B)<n → AB לא הפיכה → det(AB)=0=det(A)·0 ✓','מקרה 2: B הפיכה → B=E₁E₂···Eₖ (מכפלת אלמנטריות)','למה: det(EA)=det(E)·det(A) לכל E אלמנטרית','det(AB) = det(A)·det(E₁)···det(Eₖ) = det(A)·det(B) ∎'],
    source:'הרצאות 21-22 · לא נבחן במועד א\'!',
  },
  {
    id:5, name:'Row Rank = Col Rank = Rank', prob:70,
    conditions:['A ∈ M_{m×n}(F)'],
    flow:['חלק 1 — Row Rank = Rank: דרג ל-C, שורות ≠0 של C בסיס ל-Row(C), מספרן = rank','פעולות שורה משמרות Row(A) → Row(A)=Row(C)','חלק 2 — Col Rank = Rank: עמודות pivot בת"ל ופורשות Col(A)','מספרן = rank(A) → dim(Col A) = rank(A) ∎'],
    source:'הרצאות 16-17',
  },
  {
    id:6, name:'למת שטייניץ (החלפה)', prob:70,
    conditions:['B בסיס עם n וקטורים','S קבוצה פורשת עם m וקטורים'],
    flow:['הוכחה: בשלילה — אם n > m','B = {v₁,...,vₙ} בת"ל, S = {w₁,...,wₘ} פורשת','v₁ ∈ Span(S) → החלף w_j ב-v₁, עדיין פורשת','חזור: v₂,...,vₙ מחליפים עוד איברי S','אחרי m החלפות, לא נשארו w_j אבל עוד יש v_i → סתירה','מסקנה: n ≤ m, ולכן כל בסיסים באותו גודל ∎'],
    source:'הרצאות 11-12',
  },
];
...
const plan: DayPlan[] = [
  { day:1, date:'1.4', weekday:'רביעי', title:'Rank-Nullity + טבלת הפיכות', blocks:[
    { time:'20:00-21:00', task:'קרא QUICK_REFERENCE_6_PROOFS: משפטים 1+2. הבן את הלוגיקה.', hours:1 },
    { time:'21:00-22:00', task:'כתוב את שתי ההוכחות מזיכרון על דף ריק. השווה. חזור על שגיאות.', hours:1 },
  ]},
  { day:2, date:'2.4', weekday:'חמישי', title:'3 המשפטים + Steinitz', blocks:[
    { time:'20:00-21:00', task:'קרא הוכחות 3+6. דגש: הוכחה בשלילה, מנגנון ההחלפה.', hours:1 },
    { time:'21:00-21:30', task:'כתוב מזיכרון. תרגל: "אם dim(V)=5 ויש 5 וקטורים בת"ל, הוכח שזה בסיס."', hours:0.5 },
  ]},
  { day:3, date:'3.4', weekday:'שישי', title:'det(AB) + Row/Col Rank', blocks:[
    { time:'20:00-21:00', task:'קרא הוכחות 4+5. דגש: 2 המקרים של det(AB), למה על אלמנטריות.', hours:1 },
    { time:'21:00-21:30', task:'תרגול: חשב det של מטריצה אנטי-אלכסונית (מועד א\' Q5.1). פתור.', hours:0.5 },
  ]},
  { day:4, date:'4.4', weekday:'שבת', title:'מערכות עם פרמטר — שיטה', blocks:[
    { time:'20:00-21:30', task:'פתור מועד א\' שאלה 3.1 מחדש. סדר: דרג → מצא ערכי a קריטיים → סווג כל מקרה → כתוב פתרון לכל מקרה.', hours:1.5 },
  ]},
  { day:5, date:'5.4', weekday:'ראשון', title:'תת-מרחבים: Span∩W, U+W, dim', blocks:[
    { time:'20:00-21:30', task:'פתור מועד א\' Q3.2 + Q4.2 מחדש. דגש: הוכחת Span∩W={0} ו-U+W=Span{u}+W. שים לב להערות הבודק.', hours:1.5 },
  ]},
  { day:6, date:'6.4', weekday:'שני', title:'שקילות מערכות + הפיכות', blocks:[
    { time:'20:00-21:00', task:'למד: P הפיכה → Nul(PA)=Nul(A). פתור Q4.1 מחדש. תרגל: "אם PA=0 ו-P הפיכה, מה נובע?"', hours:1 },
    { time:'21:00-21:30', task:'חזרה על 6 המשפטים — כתוב flow של כל אחד בשורה אחת.', hours:0.5 },
  ]},
  { day:7, date:'7.4', weekday:'שלישי', title:'מבחן תרגול B2025', blocks:[
    { time:'20:00-21:30', task:'פתור Moed B 2024-25 בתנאי בחינה (1.5 שעות, 4 מתוך 5). בדוק תשובות.', hours:1.5 },
  ]},
  { day:8, date:'8.4', weekday:'רביעי', title:'ניתוח טעויות + חזרה ממוקדת', blocks:[
    { time:'20:00-21:00', task:'בדוק B2025: מה טעית? איזה משפט חסר? תרגל את הסוג שנכשל.', hours:1 },
    { time:'21:00-21:30', task:'חזרה: 6 הוכחות — כתוב כל אחת בשורה אחת (flow בלבד).', hours:0.5 },
  ]},
  { day:9, date:'9.4', weekday:'חמישי', title:'מבחן תרגול B2024', blocks:[
    { time:'20:00-21:30', task:'פתור Moed B 2023-24. דגש: שאלות הוכחה (rank-nullity? הפיכות?)', hours:1.5 },
  ]},
  { day:10, date:'10.4', weekday:'שישי', title:'דטרמיננטות — תרגול ממוקד', blocks:[
    { time:'20:00-21:00', task:'תרגל: det אנטי-אלכסונית, det בלוקים, det(AB), det(Aᵗ)=det(A). פתור 5 תרגילים.', hours:1 },
    { time:'21:00-21:30', task:'חזרה על det(AB) proof — כתוב 2 המקרים מזיכרון.', hours:0.5 },
  ]},
  { day:11, date:'11.4', weekday:'שבת', title:'מבחן תרגול B2023', blocks:[
    { time:'20:00-21:30', task:'פתור Moed B 2022-23. 1.5 שעות. בדוק.', hours:1.5 },
  ]},
  { day:12, date:'12.4', weekday:'ראשון', title:'סימולציה 2026 + חולשות', blocks:[
    { time:'20:00-21:00', task:'פתור סימולציה 2026 (אם לא פתרת). בדוק.', hours:1 },
    { time:'21:00-21:30', task:'מיקוד ב-2 נושאים הכי חלשים. תרגול ממוקד.', hours:0.5 },
  ]},
  { day:13, date:'13.4', weekday:'שני', title:'חזרה סופית — 6 משפטים + חישובים', blocks:[
    { time:'20:00-21:00', task:'כתוב 6 ההוכחות מזיכרון ברצף. תקן שגיאות.', hours:1 },
    { time:'21:00-21:30', task:'תרגול מהיר: inverse, det, דירוג מערכת עם פרמטר.', hours:0.5 },
  ]},
  { day:14, date:'14.4', weekday:'שלישי', title:'ערב מבחן — חזרה קלה בלבד', blocks:[
    { time:'20:00-21:00', task:'קרא QUICK_REFERENCE_6_PROOFS. מעבר על flow של כל משפט. לא לפתור שאלות חדשות.', hours:1 },
  ]},
];
                ...
                <button onClick={()=>setExpThm(p=>({...p,[t.id]:!p[t.id]}))} className="w-full flex items-center justify-between p-4 hover:bg-slate-50 transition-colors">
                  <div className="flex items-center gap-3 text-right">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-500">{t.prob}% סיכוי</div>
                    </div>
                  </div>
